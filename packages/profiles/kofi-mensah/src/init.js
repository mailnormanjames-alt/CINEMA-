/* Kofi Mensah — Cinematographer Init
   Chiaroscuro Light · Warm · Golden Hour */
...
(function () {
  ...
  function tick() {
    loaded++;
    if (countEl) countEl.textContent = Math.round((loaded / total) * 100);
    if (barEl) barEl.style.width = `${(loaded / total) * 100}%`;
  ...
  function done() {
    ...
    setTimeout(() => {
      if (preloaderEl) {
        gsap.to(preloaderEl, { opacity: 0, duration: 0.8, ease: 'power2.inOut', onComplete: () => { preloaderEl.remove(); } });
      }
      initHero(); initScrollAnimations(); initLenses(); initMagnetic();
    }, 500);
  }
  ...
  function initHero() {
    ...
    tl.from('.hero-eyebrow', { opacity: 0, x: -20, duration: 1, ease: 'power3.out' })
      .from('.hero-title .word', { y: 80, opacity: 0, duration: 1.2, stagger: 0.1, ease: 'power3.out' }, '-=0.6')
      .from('.hero-subtitle', { opacity: 0, y: 20, duration: 0.8, ease: 'power3.out' }, '-=0.4')
      .from('.hero-meta-item', { opacity: 0, y: 15, duration: 0.6, stagger: 0.08, ease: 'power3.out' }, '-=0.3')
      .from('.hero-scroll-cue', { opacity: 0, duration: 0.8, ease: 'power3.out' }, '-=0.2');

    gsap.to('.hero-light-leak', {
      opacity: 0.04,
  ...
  function initScrollAnimations() {
    ...
    if (quote) {
      gsap.fromTo(quote, { opacity: 0, y: 50 },
        { scrollTrigger: { trigger: '.philosophy', start: 'top 70%', end: 'top 30%', scrub: 1 }, opacity: 1, y: 0 });
    }
    ...
    document.querySelectorAll('.gear-title .word').forEach((word, i) => {
      gsap.fromTo(word, { y: 60, opacity: 0 },
        { scrollTrigger: { trigger: '.gear', start: 'top 65%', toggleActions: 'play none none none' }, y: 0, opacity: 1, duration: 1, delay: i * 0.1, ease: 'power3.out' });
    });
  ...
  function initLenses() {
    document.querySelectorAll('.lens-card').forEach((card) => {
      card.addEventListener('mouseenter', () => {
        gsap.to(card.querySelector('.lens-card-ring'), { scale: 1.15, duration: 0.6, ease: 'power2.out' });
      });
      card.addEventListener('mouseleave', () => {
        gsap.to(card.querySelector('.lens-card-ring'), { scale: 1, duration: 0.6, ease: 'power3.out' });
      });
    });
  }
